...
public class ShoppingCartServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(uri.endsWith("addbooks")) {
			...
			/*
			 * PrintWriter out = response.getWriter(); out.println(uri);
			 * out.println("<html><body>"); out.println("<h4>Selected Books</h4>");
			 * out.println("<ul>");
			 * Stream.of(selectedBooks).map(s->"<li>"+s+"</li>").forEach(out::println);
			 * out.println("</ul>");
			 * 
			 * out.println("<h4>Selected Cars</h4>"); out.println("<ul>");
			 * Stream.of(selectedCars).map(s->"<li>"+s+"</li>").forEach(out::println);
			 * out.println("</ul>");
			 * 
			 * out.println("<h4>Selected Bikes</h4>"); out.println("<ul>");
